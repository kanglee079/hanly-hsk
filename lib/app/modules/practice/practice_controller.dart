...
class PracticeController extends GetxController {
  ...
  final RxList<Map<String, dynamic>> wrongMatchAttempts = <Map<String, dynamic>>[].obs;
  ...
  Future<void> _loadSession() async {
    ...
    try {
        ...
        switch (mode) {
          case PracticeMode.learnNew:
            ...
            if (today.isNewQueueLocked) {
              ...
              if (today.isBlockedByReviewOverload) {
                ...
                noDataMessage.value = '⚠️ ${info?.message ?? "Có ${today.reviewQueue.length} từ cần ôn tập!"}\n\n'
                    'Hãy ôn tập để tiếp tục học từ mới.';
              } else if (today.isBlockedByMastery) {
                final req = today.unlockRequirement;
                noDataMessage.value = '🎯 ${req?.message ?? "Cần master từ đã học!"}\n\n'
                    'Hãy ôn tập để master ${req?.wordsToMaster ?? 0} từ còn lại.';
              } else {
                noDataMessage.value = '⚠️ Chưa thể học từ mới.\n\n${today.lockMessage}';
              }
            ...
            final actualLearned = localLearnedCount > beLearnedCount ? localLearnedCount : beLearnedCount;
            final actualRemaining = today.dailyNewLimit - actualLearned;
            
            if (actualRemaining <= 0) {
              hasNoData.value = true;
              noDataMessage.value = 'Bạn đã học đủ ${today.dailyNewLimit} từ mới hôm nay! 🎉\n\nNâng cấp Premium để học không giới hạn!';
              isLoading.value = false;
              return;
            }
            ...
            final toTake = actualRemaining < config.vocabCount ? actualRemaining : config.vocabCount;
          ...
          case PracticeMode.reviewToday:
            ...
            if (storedVocabsJson.isNotEmpty) {
              ...
              Logger.d('PracticeController', '🧠 ReviewToday: loaded ${vocabs.length} vocabs from local storage');
              break;
            }
            ...
            hasNoData.value = true;
            noDataMessage.value = 'Bạn đã hoàn thành mục tiêu hôm nay! 🎉\n\nHãy quay lại ngày mai để học tiếp.';
            isLoading.value = false;
            return;
          case PracticeMode.listening:
          case PracticeMode.matching:
          ...
          case PracticeMode.game30s:
            ...
            if (learnedVocabs.isEmpty) {
              hasNoData.value = true;
              noDataMessage.value = 'Bạn cần học từ mới trước!\n\nGame 30s chỉ dùng từ bạn đã học.';
              isLoading.value = false;
              return;
            }
      ...
      Logger.d('PracticeController', '📚 Generated ${exercises.length} exercises (preserveOrder: $shouldPreserveOrder):');
      for (int i = 0; i < exercises.length && i < 5; i++) {
        final ex = exercises[i];
        final vocab = getVocabById(ex.vocabId);
        final audioPreview = ex.questionAudioUrl != null && ex.questionAudioUrl!.length > 30
            ? '${ex.questionAudioUrl!.substring(0, 30)}...'
            : ex.questionAudioUrl ?? 'null';
        Logger.d('PracticeController', '  [$i] ${ex.type.name}: ${vocab?.hanzi ?? ex.vocabId} (audio: $audioPreview)');
      }
  ...
  void selectAnswer(int index) {
    ...
    if (mode == PracticeMode.learnNew) {
      ...
      _learnNewTotalByVocab[vocabId] = (_learnNewTotalByVocab[vocabId] ?? 0) + 1;
      if (isCorrect.value) {
        _learnNewCorrectByVocab[vocabId] = (_learnNewCorrectByVocab[vocabId] ?? 0) + 1;
      }
      final spent = _exerciseStart != null
          ? DateTime.now().difference(_exerciseStart!).inMilliseconds
          : 3000;
      _learnNewTimeSpentMsByVocab[vocabId] = (_learnNewTimeSpentMsByVocab[vocabId] ?? 0) + spent;
    }
    ...
    if (mode == PracticeMode.game30s) {
      ...
      state.value = PracticeState.feedback;
    }
  }
  ...
  void _checkMatchingPair() {
    ...
    if (isMatch) {
      ...
      final leftItem = exercise.matchingItems!.firstWhere((i) => i.leftIndex == leftIdx);
      final rightItem = exercise.matchingItems!.firstWhere((i) => i.rightIndex == rightIdx);
      final correctRightItem = exercise.matchingItems!.firstWhere((i) => i.leftIndex == leftIdx);
  ...
  Future<void> playAudio({bool slow = false}) async {
    ...
    if (exercise != null) {
      ...
      Logger.d('PracticeController', '🔊 Audio request: vocabId=${exercise.vocabId}, slow=$slow, url=$url');
    } else {
      ...
      if (vocab != null) {
        ...
        Logger.d('PracticeController', '🔊 Audio (vocab fallback): vocabId=${vocab.id}, slow=$slow, url=$url');
      }
    }
  ...
  void _evaluatePronunciation(String spoken) {
    ...
    // Simple evaluation
    final match = spoken.contains(vocab.hanzi) || 
                  vocab.hanzi.contains(spoken);
  ...
  void skipPronunciation() {
    hasPronunciationResult.value = false;
    state.value = config.useSRSRating ? PracticeState.rating : PracticeState.exercise;
    nextExercise();
  }
  ...
  Future<void> _finishSession() async {
    ...
    try {
      ...
      // Push realtime updates to Today/Learn/Me immediately after session ends.
      unawaited(_rt.syncNowKeys(
        const ['today', 'todayForecast', 'learnedToday', 'studyModes'],
        force: true,
      ));
    }
  }
    ...
    try {
      ...
      switch (mode) {
        ...
        case PracticeMode.reviewToday:
          ...
          newVocabs = allVocabs.where((v) => learnedIds.contains(v.id)).take(config.vocabCount).toList();
          break;
        default:
          final combined = [...today.reviewQueue, ...today.newQueue];
          combined.shuffle();
  ...
  void _initLearnNewLocalProgress() {
    ...
    if (mode != PracticeMode.learnNew && mode != PracticeMode.reviewToday) return;
    try {
  ...
  Future<void> _completeLearnNewWord(String vocabId) async {
    ...
    try {
      ...
      Logger.d('PracticeController', '✅ LearnNew progress submitted: vocabId=$vocabId rating=${rating.value} ratio=${ratio.toStringAsFixed(2)}');
    } catch (e) {
      Logger.e('PracticeController', '❌ LearnNew submit progress error (vocabId=$vocabId)', e);
